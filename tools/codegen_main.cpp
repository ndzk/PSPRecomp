...
int generate_relocatable(const std::filesystem::path &image_path,
                         std::uint32_t base,
                         std::uint32_t unit_index,
                         const std::filesystem::path &output_dir) {
    std::ifstream input(image_path, std::ios::binary);
    if (!input) throw psprecomp::Error("could not open " + image_path.string());
    const std::vector<std::uint8_t> image((std::istreambuf_iterator<char>(input)),
                                          std::istreambuf_iterator<char>());
    if (image.empty() || (image.size() % 4u) != 0u) {
        throw psprecomp::Error("a code image must be a non-empty whole number of instructions");
    }
    if ((base % 4u) != 0u) throw psprecomp::Error("the base address must be aligned");

    psprecomp::GuestMemory memory;
    if (!memory.contains(base, image.size())) {
        throw psprecomp::Error("the image does not fit in guest memory at that base");
    }
    ...
    g_relocatable = RelocatableUnit{true, base, limit, unit_index};

    std::ostringstream suffix;
    suffix << std::setfill('0') << std::setw(4) << unit_index;
    ...
    source << "// Generated by psp_recomp --relocatable. Do not edit.\n"
           << "//\n"
           ...
           << "namespace psprecomp {\n\n"
           << emit_function_source(generated_unit, memory, name) << "\n";

    // The entry offsets belong with the generated code rather than in whatever
    // loads it: they are a product of the translation, and a profile repeating
    // them by hand would be one edit away from disagreeing with it.
    source << "void register_" << name << "_at(Runtime &rt, std::uint32_t base) {\n"
           << "    rt.set_unit_base(" << unit_index << "u, base);\n";
    for (const auto entry : entries) {
        source << "    rt.register_function(base + " << psprecomp::hex32(entry - base)
               << "u, &" << name << ", \"" << name << "+"
               << psprecomp::hex32(entry - base) << "\");\n";
    }
    source << "}\n\n} // namespace psprecomp\n";

    const auto path = output_dir / ("generated_relocatable_" + suffix.str() + ".cpp");
    (void)write_text_if_changed(path, source.str());
    ...
    std::cout << "  image:            " << image.size() << " bytes at "
              ...
              << "  register with:    register_" << name << "_at(rt, base)\n"
              << "  output:           " << path.string() << "\n";
    return 0;
}
...
int main(int argc, char **argv) {
    try {
        ...
        if (argc == 6 && std::string_view(argv[1]) == "--relocatable") {
            const auto base = static_cast<std::uint32_t>(std::stoul(argv[3], nullptr, 0));
            const auto unit = static_cast<std::uint32_t>(std::stoul(argv[4], nullptr, 0));
            return generate_relocatable(argv[2], base, unit, argv[5]);
        }
        ...
        std::cerr << "Usage:\n"
                  ...
                  << "  psp_recomp --relocatable <image.bin> <base_hex> <unit_index> <generated_dir>\n";
        return 2;
