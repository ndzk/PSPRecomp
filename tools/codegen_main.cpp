...
// Every machine-generated file states what produced it.
//
// Paths are deliberately absent.  A path changes no byte of the output, and
// embedding one would make two machines that generated the same corpus from the
// same input disagree file for file, which costs the property that makes a
// checked-in corpus auditable at all: regenerate it and the bytes must match.
// The SHA-256 pins the input exactly, and the parameters below are the only
// inputs besides the image itself that change what is emitted.  No timestamp,
// for the same reason.
std::string provenance_header(std::string_view mode,
                              const std::string &input_sha256,
                              std::string_view invocation) {
    std::ostringstream out;
    out << "// Generated by psp_recomp (" << mode << ").  Do not edit:\n"
...
int generate_manual(const std::filesystem::path &elf_path,
    ...
    out << provenance_header("manifest", psprecomp::sha256_file(elf_path),
...
int generate_relocatable(const std::filesystem::path &image_path,
    ...
    std::ostringstream relocatable_invocation;
    relocatable_invocation << "psp_recomp --relocatable <image.bin> " << psprecomp::hex32(base)
                           << ' ' << unit_index << " <generated_dir>";
    source << provenance_header("--relocatable", psprecomp::sha256_file(image_path),
                                relocatable_invocation.str())
...
int generate_auto(const std::filesystem::path &elf_path,
                  ...
                  std::uint32_t unit_span_bytes) {
    const auto elf = psprecomp::Elf32Image::from_file(elf_path);
    std::ostringstream auto_invocation;
    auto_invocation << "psp_recomp <ELF> --auto <generated_dir> " << psprecomp::hex32(load_base)
                    << ' ' << unit_span_bytes;
    const std::string input_sha256 = psprecomp::sha256_file(elf_path);
    const std::string header = provenance_header("--auto", input_sha256, auto_invocation.str());
    psprecomp::GuestMemory memory;
    (void)elf.load_and_relocate(memory, load_base);
    std::vector<psprecomp::PspImport> imports;
    if (const auto module = elf.find_module_info(memory, load_base)) imports = elf.scan_imports(memory, *module);
    ...
    units_header << header << "#pragma once\n\n#include <cstdint>\n#include \"psprecomp/guest_memory.hpp\"\n\nnamespace psprecomp {\nclass Runtime;\nstruct AllegrexContext;\n";
    for (const auto &unit : units) {
    ...
    for (const auto &unit : units) {
        ...
        std::ostringstream out;
        out << header << "#include \"psprecomp/runtime.hpp\"\n#include \"generated_units.hpp\"\n#include <bit>\n#include <cmath>\n#include <cstdint>\n#include <limits>\n\nnamespace psprecomp {\n";
    ...
    registry << header << "#include \"psprecomp/runtime.hpp\"\n#include <cstdint>\n\nnamespace psprecomp {\n";
